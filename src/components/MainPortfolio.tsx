  ...
  const renderSectionsMap: Record<string, React.ReactNode> = {
    hero: (
      <section
        key="hero"
        id="hero"
        className="relative min-h-screen pt-24 pb-16 flex items-center justify-center overflow-hidden z-10"
      >
        {profile.heroImage && (
        ...
        <div className="relative max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 z-10 select-none text-center">
          <div className="max-w-4xl mx-auto space-y-8">
            <motion.div
              ...
              className="inline-flex items-center gap-2.5 px-4.5 py-1.5 bg-white/70 dark:bg-zinc-900/60 border border-zinc-200/50 dark:border-zinc-800/80 rounded-full text-xs font-mono font-bold text-zinc-700 dark:text-zinc-300 shadow-sm backdrop-blur-md"
            >
            ...
            <div className="space-y-4">
              ...
              <motion.p
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={getFramerTransition(profile.motionStrength)}
                className="max-w-2xl mx-auto text-sm sm:text-base md:text-lg text-zinc-500 dark:text-zinc-400 font-sans font-light leading-relaxed pt-2"
              >
              ...
              <div className="flex items-center gap-3 select-none">
                <div className="flex -space-x-2.5">
                  {[
                    ...
                    <div
                      key={i}
                      className={`w-8 h-8 rounded-full border-2 border-white dark:border-[#000] bg-gradient-to-tr ${gradient} flex items-center justify-center text-[9px] font-mono font-bold text-white shadow-sm`}
                    >
                      {["JD", "MK", "AL", "HR"][i]}
                    </div>
                  ))}
                </div>
                <div className="text-left font-mono text-[10px] text-zinc-400 dark:text-zinc-500 font-bold tracking-wider uppercase">
                  TRUSTED BY LEADERS
                </div>
              </div>
            </motion.div>
            ...
            {bentoCards.map((card, index) => {
              return (
                <motion.div
                  ...
                  className={`group relative h-[280px] sm:h-[320px] rounded-3xl overflow-hidden ${getCardStyle(profile.effectsStyle)} cursor-pointer clickable select-none transition-all duration-500 text-left flex flex-col justify-between p-8`}
                  onClick={() => navigate(`/work/${card.category}`)}
                >
                  <div className={`absolute inset-0 ${card.glowClass} pointer-events-none z-0`} />
                  ...
                  <div className="relative z-10 flex justify-between items-start">
                    ...
                    <span className="w-10 h-10 rounded-full bg-zinc-100 hover:bg-zinc-200 text-zinc-800 dark:bg-white/10 dark:text-white dark:group-hover:bg-primary dark:group-hover:text-white border border-zinc-200 dark:border-transparent flex items-center justify-center transition-all duration-300 group-hover:rotate-45">
    ...
    videoReels: profile.showVideoReel ? (
      <section key="videoReels" id="videoReels" className={`${getSectionPadding(profile.sectionSpacing)} relative z-10`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-left max-w-xl space-y-4 mb-12 select-none">
            <span className={getChipStyle(profile.chipsStyle)}>
    ...
    skills: profile.showSkills ? (
      <section key="skills" id="skills" className={`${getSectionPadding(profile.sectionSpacing)} relative z-10 border-t border-zinc-200/30 dark:border-zinc-800/30`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mt-8 space-y-8 select-none text-left">
            <h3 className="text-lg font-mono font-bold tracking-wider text-zinc-400 dark:text-zinc-500 uppercase">
              PROFESSIONAL CORE UTILITIES
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {SKILL_CATEGORIES.map((cat, catIdx) => (
                <div
                  key={cat.title}
                  className={`p-6 rounded-3xl space-y-6 border border-zinc-200/50 dark:border-zinc-800/50 ${getCardStyle(profile.effectsStyle)}`}
                >
                  <h4 className="text-sm font-mono font-bold text-zinc-900 dark:text-white border-b border-zinc-200 dark:border-zinc-800 pb-3 uppercase tracking-wider">
                    {cat.title}
                  </h4>

                  <ul className="space-y-4">
                    {cat.skills.map((skill, skillIdx) => (
                      <li key={skill.name} className="space-y-2 group">
                        <div className="flex justify-between text-xs font-mono">
                          <span className="text-zinc-700 dark:text-zinc-300 font-medium group-hover:text-primary transition-colors">
                            {skill.name}
                          </span>
                        ...
                        <div className="h-1 bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden">
                          <motion.div
                            ...
                            className="h-full bg-gradient-to-r from-primary to-zinc-500 dark:to-zinc-400"
                          />
                        </div>
                      </li>
                    ))}
    ...
    experience: profile.showExperience && experiences.length > 0 ? (
      <section key="experience" id="experience" className={`${getSectionPadding(profile.sectionSpacing)} bg-zinc-50/50 dark:bg-zinc-900/10 relative z-10`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-left max-w-xl space-y-4 mb-16 select-none">
            <span className={getChipStyle(profile.chipsStyle)}>
          ...
          <div className="max-w-4xl mx-auto space-y-12 relative before:absolute before:top-0 before:bottom-0 before:left-4 md:before:left-1/2 before:-translate-x-0.5 before:w-px before:bg-zinc-200 dark:before:bg-zinc-800">
            {experiences.map((exp, index) => {
              ...
              return (
                ...
                >
                  <div className="absolute left-4 md:left-1/2 -translate-x-1.5 md:-translate-x-2.5 w-4 h-4 md:w-5 md:h-5 rounded-full bg-slate-50 dark:bg-zinc-950 border-4 border-primary z-10" />
                  <div className="hidden md:block w-1/2 px-8" />

                  <div className={`w-full md:w-1/2 pl-12 text-left ${isEven ? "md:pl-0 md:pr-8" : "md:pl-8 md:pr-0"}`}>
                    <div className={`p-6 rounded-3xl space-y-4 hover:border-primary/20 hover:shadow-md transition-all duration-300 ${getCardStyle(profile.effectsStyle)}`}>
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-zinc-200/50 dark:border-zinc-800/50 pb-3">
                        <div>
                          <h3 className="text-lg font-display font-semibold text-zinc-900 dark:text-white leading-tight">
                            {exp.role}
                          </h3>
                          <p className="text-xs font-mono font-medium text-primary mt-0.5 uppercase tracking-wide">
                            {exp.company}
                          </p>
                        </div>
                        <span className="shrink-0 px-3 py-1 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/50 rounded-full text-[10px] font-mono font-semibold text-zinc-500 dark:text-zinc-400">
                          {exp.duration}
                        </span>
                      </div>

                      <ul className="space-y-2.5 text-xs md:text-sm text-zinc-500 dark:text-zinc-400 font-sans font-light leading-relaxed">
