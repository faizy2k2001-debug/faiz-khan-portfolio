...
export default function ThreeDHeroCanvas() {
  ...
  useEffect(() => {
    ...
    // 4 Elegant waves mimicking V7 Labs aesthetic
    const waves: Wave[] = [
      {
    ...
    // Flowing particles along the paths
    ...
    const getWaveY = (x: number, wave: Wave, time: number) => {
      ...
      const borderEnvelope = Math.sin((x / width) * Math.PI);
      ...
      const verticalOffsetFactor = Math.pow(1 - Math.sin((x / width) * Math.PI), 1.5);
